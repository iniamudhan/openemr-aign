...
class Installer
{
    ...
    public function create_database_user()
    {
		...
		echo "CREATE USER '" . $this->escapeSql($this->login) . "@'%' IDENTIFIED BY '" . $this->escapeSql($this->pass) . "'";
                return $this->execute_sql("CREATE USER '" . $this->escapeSql($this->login) . "'@'%' IDENTIFIED BY '" . $this->escapeSql($this->pass) . "' REQUIRE X509");
		  ...
		  echo "CREATE USER '" . $this->escapeSql($this->login) . "@'%' IDENTIFIED BY '" . $this->escapeSql($this->pass) . "'";
                return $this->execute_sql("CREATE USER '" . $this->escapeSql($this->login) . "'@'%' IDENTIFIED BY '" . $this->escapeSql($this->pass) . "' REQUIRE SSL");
	    } else {
		 echo "CREATE USER '" . $this->escapeSql($this->login) . "@'%' IDENTIFIED BY '" . $this->escapeSql($this->pass) . "'";
                return $this->execute_sql("CREATE USER '" . $this->escapeSql($this->login) . "'@'%' IDENTIFIED BY '" . $this->escapeSql($this->pass) . "'");
            }
        }
    }
    ...
    {
	    echo "GRANT ALL PRIVILEGES ON " . $this->escapeDatabaseName($this->dbname) . ".* TO '" . $this->escapeSql($this->login) . "'@'%'";
        return $this->execute_sql("GRANT ALL PRIVILEGES ON " . $this->escapeDatabaseName($this->dbname) . ".* TO '" . $this->escapeSql($this->login) . "'@'%'");
    }
