...
(function (define) {
    define(["jquery"], function ($, root) {
        ...
        function alert(data, title) {
            ...
            let alertTitle =
                '<span class="text-danger bg-light"><i class="fa fa-exclamation-triangle"></i>&nbsp;' +
                title +
                "</span>";
            return dlgopen("", "", 675, 0, "", alertTitle, {
                buttons: [{ text: "OK", close: true, style: "primary" }],
                type: "Alert",
                sizeHeight: "auto",
        ...
        function confirm(data, title) {
            ...
            let alertTitle =
                '<span class="text-info bg-light"><i class="fa fa-exclamation-triangle"></i>&nbsp;' +
                title +
                "</span>";
        ...
        function popUp(url, data, name, width, height) {
            ...
            if (
                ...
                width = parseInt(
                    (window.screen.width * parseInt(width, 10)) / 100,
                    10
                );
            }
            if (
            ...
            return new Promise(function (resolve, reject) {
                if (type == "script") {
                    ...
                    newScriptElement.onerror = () =>
                        reject(new Error(`Script load error for ${srcUrl}`));
                ...
                if (type === "link") {
                    ...
                    newScriptElement.onload = () => resolve(newScriptElement);
                    newScriptElement.onerror = () =>
                        reject(new Error(`Link load error for ${srcUrl}`));
...
function cascwin(url, winname, width, height, options) {
    ...
    if (navigator.appName == "Microsoft Internet Explorer") {
        ...
        if (re.exec(ua) != null) rv = parseFloat(RegExp.$1); // this holds the version number
        height = height + 28;
    }
...
if (typeof alertMsg !== "function") {
    function alertMsg(
        ...
        let oSize =
            size == "lg" ? "left:10%;width:80%;" : "left:25%;width:50%;";
        let style =
            "position:fixed;top:25%;" + oSize + " bottom:0;z-index:9999;";
        $("body").prepend(
            "<div class='container text-center' id='alert_box' style='" +
                style +
                "'></div>"
        );
        let mHtml =
            '<div id="alertmsg" class="alert alert-' +
            type +
            ' alert-dismissable">' +
            '<button type="button" class="btn btn-link ' +
            oHidden +
            '" id="dontShowAgain" data-dismiss="alert">' +
            gotIt +
            ...
            message +
            "</p>" +
            '<button type="button" id="alertDismissButton" class="pull-right btn btn-link" data-dismiss="alert">' +
            dismiss +
            "</button><br /></div>";
...
function dlgopen(url, winname, width, height, forceNewWindow, title, opts) {
    ...
    jQuery(function () {
        ...
        let bscssRtl =
            top.webroot_url +
            "/public/assets/bootstrap-v4-rtl/dist/css/bootstrap-rtl.min.css";
        let bsurl =
            top.webroot_url +
            "/public/assets/bootstrap/dist/js/bootstrap.bundle.min.js";
    ...
    if (Math.abs(width) > 0) {
        ...
        msSize =
            "<style>.modal-custom-" +
            winname +
            " {max-width:35% !important;}</style>"; // standard B.S. modal default (modal-md)
    }
    ...
    if (mSize === "modal-sm") {
        ...
        msSize =
            "<style>.modal-custom-" +
            winname +
            " {max-width:40% !important;}</style>";
    } else if (mSize === "modal-mlg") {
        msSize =
            "<style>.modal-custom-" +
            winname +
            " {max-width:55% !important;}</style>";
    } else if (mSize === "modal-lg") {
        msSize =
            "<style>.modal-custom-" +
            winname +
            " {max-width:75% !important;}</style>";
    ...
    var frameHtml =
        '<iframe id="modalframe" class="modalIframe w-100 h-100 border-0" name="%winname%" %url%></iframe>'
            .replace("%winname%", winname)
            .replace("%url%", fullURL ? "src=" + fullURL : "");

    var contentStyles = 'style="height:%initHeight%; max-height: 94vh"'.replace(
        "%initHeight%",
        opts.sizeHeight !== "full" ? mHeight : "90vh"
    );
    ...
    var mhtml = (
        ...
        '<div class="modal-content %resize-action%" %contentStyles%>' +
        "%head%" +
        "%altclose%" +
        "%wait%" +
        '<div class="modal-body px-1 h-100">' +
        "%body%" +
        "</div></div></div></div>"
    )
        .replace("%id%", winname)
        ...
        if (opts.buttons) {
            for (var i = 0, k = opts.buttons.length; i < k; i++) {
                ...
                if (typeof btnOp.class !== "undefined") {
                    ...
                    var btn = jQuery("<button>").addClass(
                        "btn " + (btnOp.class || "btn-primary")
                    );
                } else {
                    // legacy
                    btnOp.style = btnOp.style.replace(/default/gi, "secondary");
                    var btn = jQuery("<button>").addClass(
                        "btn btn-" + (btnOp.style || "primary")
                    );
                    btnOp.style = "";
                }
    ...
    function sizing(e, height) {
        ...
        frameContentHt =
            frameContentHt >= viewPortHt ? viewPortHt : frameContentHt;
    ...
    function SizeModaliFrame(e, minSize) {
        ...
        try {
            ...
            jQuery(e.currentTarget)
                ...
                .css({ height: 0 });
            frameContentHt =
                Math.max(jQuery(idoc).height(), idoc.body.offsetHeight) + 60;
        } catch (err) {
